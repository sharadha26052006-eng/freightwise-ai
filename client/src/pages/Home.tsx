...
function ScenariosView() {
  ...
  const scenarios = useMemo(() => [{ name: "Ship now", detail: "Supramax · May 15", freight: 4.284, landed: 15.684, voyages: 3, quality: "Illustrative" }, { name: "Ship later", detail: "Supramax · Jun 15", freight: later ? 4.452 : 4.626, landed: later ? 15.852 : 16.026, voyages: 3, quality: "Illustrative" }, { name: "Change vessel", detail: "Panamax · May 15", freight: 3.948, landed: 15.348, voyages: 2, quality: "Illustrative" }], [later]);
  return <div className="view-stack"><div className="page-heading"><div><div className="breadcrumb"><span>Planning</span><MoveRight size={14} /><strong>Scenarios</strong></div><h1>Compare the decision paths</h1><p>See the numerical differences; the tool does not declare a universal “best” scenario.</p></div><DataQuality /></div><EmptyNotice /><Panel title="Scenario assumptions" subtitle="Adjust the timing input to make the comparison tangible"><div className="scenario-controls"><div className="control-chip"><Clock3 size={16} /><span><strong>Current window</strong><small>May 15, 2026</small></span></div><button className={`toggle ${later ? "toggle-on" : ""}`} onClick={() => setLater((value) => !value)} aria-label="Toggle later market case"><span /></button><span className="control-copy"><strong>Model a later-rate case</strong><small>{later ? "Later case uplift is active" : "Uses the same baseline, for clarity"}</small></span></div></Panel><Panel title="Scenario comparison" subtitle="120,000 tonnes · Newcastle → Paradip"><div className="scenario-grid">{scenarios.map((scenario, index) => <div className={`scenario-card ${index === 0 ? "scenario-selected" : ""}`} key={scenario.name}><div className="scenario-card-top"><span className="scenario-index">0{index + 1}</span>{index === 0 && <span className="recommended-label">Current plan</span>}</div><h3>{scenario.name}</h3><p>{scenario.detail}</p><div className="scenario-number"><span>Freight</span><strong>₹{scenario.freight.toFixed(3)}M</strong></div><div className="scenario-number"><span>Landed cost</span><strong>₹{scenario.landed.toFixed(3)}M</strong></div><div className="scenario-footer"><span>{scenario.voyages} voyages</span><DataQuality level={scenario.quality} /></div></div>)}</div></Panel><Panel title="Cost comparison" subtitle="Same cargo purchase and other cost assumptions across all cases"><div className="chart scenario-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={scenarios} margin={{ top: 8, right: 12, bottom: 0, left: -12 }}><CartesianGrid vertical={false} stroke="#e4eceb" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: "#506a6d", fontSize: 11 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: "#7b8f91", fontSize: 11 }} tickFormatter={(value) => `₹${value}M`} /><Tooltip formatter={(value: number) => [`₹${value.toFixed(3)}M`, "Landed cost"]} contentStyle={{ borderRadius: 12, border: "1px solid #dbe8e6", fontSize: 12 }} /><Bar dataKey="landed" fill="#2c9a92" radius={[5, 5, 0, 0]} barSize={42} /></BarChart></ResponsiveContainer></div><div className="comparison-note"><Info size={15} /><span>The change-vessel scenario shows a lower illustration because Panamax requires fewer voyages. Validate draft, berth, and cargo compatibility before treating that difference as actionable.</span></div></Panel></div>;
}
